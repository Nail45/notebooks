...
class MainController extends Controller
{
    public function index(Request $request): Factory|View|JsonResponse
    {
        try {
            ...
            // Текущая сортировка
            $currentSort = $sortOptions[$sort] ?? $sortOptions['default'];
            ...
            // Получаем производителя из запроса
            ...
            if ($manufacturer && $manufacturer !== 'all' && in_array($manufacturer, $allowedManufacturers)) {
                $query->where('manufacturer', $manufacturer);
            } else {
                // По умолчанию показываем все разрешенные производители
                $query->whereIn('manufacturer', $allowedManufacturers);
            }
            ...
            if ($request->has('release_year_from') && is_numeric($request->release_year_from)) {
                $query->where('release_date', '>=', (int)$request->release_year_from);
            }

            if ($request->has('release_year_to') && is_numeric($request->release_year_to)) {
                $query->where('release_date', '<=', (int)$request->release_year_to);
            }

            // Фильтр по виду
            $query->when($request->anyFilled(['worker', 'gaming', 'ultrabook', 'universal', 'domestic']),
                function ($q) use ($request) {
                    ...
                    if (!empty($viewFilters)) {
                        $q->whereIn('view_title', $viewFilters);
                    }
                }
            );

            // Фильтр по диагонали экрана
            if ($request->has('screen_diagonal_from') && is_numeric($request->screen_diagonal_from)) {
                $query->where('screen_diagonal', '>=', (float)$request->screen_diagonal_from);
            }

            if ($request->has('screen_diagonal_to') && is_numeric($request->screen_diagonal_to)) {
            ...
            // Фильтр по популярным параметрам
//            if ($request->has('keyboard_backlight')) {
//                echo $request->get('keyboard_backlight');
//            }

// Фильтрация по популярным параметрам
            if ($request->has('keyboard_backlight')) {
                // Ищем любую подсветку (значения, начинающиеся с "есть")
            ...
            if ($request->has('numeric_keypad')) {
                // Ищем любую цифровую клавиатуру (значения, начинающиеся с "есть")
                $query->where(function ($q) {
            ...
            if ($request->has('touch_screen')) {
                // Только точное значение "есть" для сенсорного экрана
                $query->where('touch_screen', '=', 'есть');
            }

            if ($request->has('hdmi_port')) {
                // Ищем HDMI (любое значение кроме "нет" и NULL)
                $query->where(function ($q) {
            ...
            if ($request->has('displayport')) {
                // Ищем DisplayPort (любое значение кроме "нет" и NULL)
                $query->where(function ($q) {
            ...
            foreach ($ramOptions as $field => $value) {
...
// Если выбраны типы ОЗУ - применяем фильтр
...
// Apple
            ...
            if (!empty($selectedProcessors)) {
                $query->where(function ($q) use ($selectedProcessors) {
                    foreach ($selectedProcessors as $processor) {
                        $q->orWhere('processor_series', 'LIKE', '%' . $processor . '%');
                    }
                });
            }
            ...
            // Получаем все поля processor_model_*
            foreach ($request->all() as $key => $value) {
                if (str_starts_with($key, 'processor_model_') && $value) {
                    // Декодируем модель из имени поля
                    $model = str_replace('_', ' ', substr($key, 16));
                    $model = str_replace(['intel_', 'amd_', 'apple_', 'snapdragon_'],
                        ['Intel ', 'AMD ', 'Apple ', 'Snapdragon '], $model);
...
// Маппинг чекбоксов на значения в БД
            $coreMapping = [
                'cores_2' => ['2'],
                'cores_4' => ['4'],
                'cores_5' => ['5'],
            ...
            foreach ($coreMapping as $field => $values) {
                if ($request->has($field)) {
...
// Также проверяем конкретные модели
            $selectedModels = [];
            foreach ($request->all() as $key => $value) {
...
// Применяем фильтр
                ...
                $query->where(function ($q) use ($selectedCategories, $selectedModels) {
                    ...
                    foreach ($selectedCategories as $category) {
                        foreach ($category['search_terms'] as $term) {
                            $q->orWhere('gpu_model', 'LIKE', '%' . $term . '%');
                        }
                    }

                    // Конкретные модели
                    foreach ($selectedModels as $model) {
                        $q->orWhere('gpu_model', 'LIKE', '%' . $model . '%');
                    }
                });
            }
            ...
            foreach ($storageMapping as $field => $values) {
                if ($request->has($field)) {
...
// Применяем фильтр с OR логикой
            ...
            foreach ($screenTechMapping as $field => $technology) {
                if ($request->has($field)) {
...
// Применяем фильтр с OR логикой
            ...
            if ($request->hasAny([
                'case_material_plastic', 'case_material_aluminum', 'case_material_metal',
                'case_material_magnesium', 'case_material_carbon',
                'case_material_plastic_metal', 'case_material_plastic_aluminum',
                'case_material_metal_plastic', 'case_material_magnesium_metal',
                'case_material_metal_magnesium', 'case_material_plastic_carbon',
                'case_material_carbon_fiber', 'case_material_pc_cf'
            ])) {
                $this->applyCaseMaterialFilter($query, $request);
            }

            // Фильтр по видеопамяти
            if ($request->filled('vram')) {
                $this->applyVramFilter($query, $request);
            }

            // Фильтр по частоте процессора
            $this->applyCpuFreqFilter($query, $request);

            // Фильтр по трансформеру
            if ($request->filled('transformer')) {
                $this->applyTransformerFilter($query, $request);
            }

            // Фильтр по емкости аккумулятора
            if ($request->filled('battery_capacities') || $request->filled('battery_range')) {
                $this->applyBatteryCapacityFilter($query, $request);
            }

            // Применяем фильтр Ethernet если есть параметр
            if ($request->filled('ethernet')) {
                $this->applyEthernetFilter($query, $request);
            }

            // Применяем фильтр USB портов
            if ($request->filled('usb_ports') && is_array($request->usb_ports)) {
                $this->applyUsbPortsFilter($query, $request);
            }

            // Применяем фильтр Thunderbolt
            if ($request->filled('thunderbolt')) {
                $this->applyThunderboltFilter($query, $request);
            }

            // Применяем фильтр кириллической клавиатуры
            if ($request->filled('cyrillic_keyboard')) {
                $this->applyCyrillicKeyboardFilter($query, $request);
            }
            ...
            // ПАГИНАЦИЯ - 40 НОУТБУКОВ НА СТРАНИЦЕ
            $perPage = 40;
            $currentPage = max(1, (int)$request->get('page', 1));

            // Получаем общее количество товаров для текущего фильтра
            $totalItems = $query->count();

            // Вычисляем общее количество страниц
            $totalPages = max(1, ceil($totalItems / $perPage));

            // Если запрошенная страница больше максимальной, возвращаем первую
            ...
            $notebooks = $query->skip($offset)->take($perPage)->get();
            ...
            // Подготавливаем параметры для пагинатора
            $appends = [];

            if ($manufacturer !== 'all') {
                $appends['manufacturer'] = $manufacturer;
            }
            ...
            // Добавляем все параметры фильтров
            $filterParams = $request->except(['page', '_token']);
            foreach ($filterParams as $key => $value) {
                if (!empty($value) && $key !== 'sort' && $key !== 'manufacturer') {
                    $appends[$key] = $value;
                }
            }
            ...
            $activeFilters = [];
            foreach ($request->except(['page', 'sort', 'manufacturer']) as $key => $value) {
                if (!empty($value)) {
                    $activeFilters[$key] = $value;
                }
            }
            // Проверяем AJAX запрос (используем строгую проверку)
            ...
            if ($isAjaxRequest) {
                ...
                if ($totalPages > 1) {
                    try {
                        $paginationHtml = $paginator->links('vendor.pagination.tailwind')->toHtml();
                    } catch (\Exception $e) {
                        // Если ошибка с пагинацией, создаем простую пагинацию
                        $paginationHtml = $this->generateSimplePagination($paginator);
                    }
                }
                ...
                try {
                    ...
                    // Если ошибка рендеринга, возвращаем пустой HTML
            ...
            // Для обычного запроса выбрасываем исключение
            throw $e;
        }
    }


// Метод для генерации простой пагинации (добавьте в класс)
    private function generateSimplePagination($paginator): string
    {
        $html = '<nav class="pagination"><ul class="flex justify-center space-x-2">';

        // Предыдущая страница
        if ($paginator->onFirstPage()) {
            $html .= '<li class="disabled px-3 py-2 text-gray-500"><span>&laquo; Назад</span></li>';
        } else {
            $html .= '<li><a href="' . e($paginator->previousPageUrl()) . '" class="px-3 py-2 text-blue-600 hover:text-blue-800" rel="prev">&laquo; Назад</a></li>';
        }
        ...
        if ($lastPage <= 7) {
            ...
            for ($i = 1; $i <= $lastPage; $i++) {
                if ($i == $currentPage) {
                    $html .= '<li class="active"><span class="px-3 py-2 bg-blue-600 text-white">' . $i . '</span></li>';
                } else {
                    $html .= '<li><a href="' . e($paginator->url($i)) . '" class="px-3 py-2 text-blue-600 hover:text-blue-800">' . $i . '</a></li>';
                }
            }
        } else {
            ...
            if ($start > 1) {
                $html .= '<li><a href="' . e($paginator->url(1)) . '" class="px-3 py-2 text-blue-600 hover:text-blue-800">1</a></li>';
                if ($start > 2) {
                    $html .= '<li><span class="px-3 py-2 text-gray-500">...</span></li>';
                }
            }
            ...
            if ($end < $lastPage) {
                ...
                $html .= '<li><a href="' . e($paginator->url($lastPage)) . '" class="px-3 py-2 text-blue-600 hover:text-blue-800">' . $lastPage . '</a></li>';
            }
        }

        // Следующая страница
        if ($paginator->hasMorePages()) {
            $html .= '<li><a href="' . e($paginator->nextPageUrl()) . '" class="px-3 py-2 text-blue-600 hover:text-blue-800" rel="next">Вперед &raquo;</a></li>';
        } else {
            $html .= '<li class="disabled px-3 py-2 text-gray-500"><span>Вперед &raquo;</span></li>';
        }
    ...
    private function slugToGpuModel($slug)
    {
    ...
    private function applyCaseMaterialFilter($query, $request)
    {
        ...
        foreach ($materialMapping as $field => $material) {
            if ($request->has($field)) {
        ...
        if (!empty($selectedMaterials)) {
            $query->where(function ($q) use ($selectedMaterials) {
                foreach ($selectedMaterials as $material) {
                    // Ищем точное совпадение или вхождение в строку
                    $q->orWhere('case_material', 'LIKE', '%' . $material . '%');
                }
            });
        }
    }
    ...
    private function applyVramFilter($query, $request)
    {
        if ($request->filled('vram')) {
            $selectedVrams = $request->input('vram');

            $query->where(function ($q) use ($selectedVrams) {
                foreach ($selectedVrams as $vram) {
                    // Ищем значения, содержащие указанный объем
                    // Например: "6 Гб", "6 GB", "6GB" и т.д.
                    $q->orWhere('gpu_memory', 'LIKE', '%' . $vram . '%');

                    // Также обрабатываем числовые значения
                    $q->orWhere('gpu_memory', 'LIKE', $vram . '%');
                    $q->orWhere('gpu_memory', 'LIKE', '%' . $vram . 'Гб%');
                    $q->orWhere('gpu_memory', 'LIKE', '%' . $vram . 'GB%');
                }
            });
        }
    }

    private function applyCpuFreqFilter($query, $request)
    {
        // Если выбран "любой" или ничего не выбрано - пропускаем фильтр
        if ($request->has('cpu_freq_range') && $request->cpu_freq_range == 'any') {
            return;
        }

        // Если указан диапазон слайдерами
        if ($request->filled('cpu_freq_min_slider') || $request->filled('cpu_freq_max_slider')) {
            $min = $request->filled('cpu_freq_min_slider') ? (float)$request->cpu_freq_min_slider * 1000 : 900; // в МГц
            $max = $request->filled('cpu_freq_max_slider') ? (float)$request->cpu_freq_max_slider * 1000 : 4500; // в МГц

            $this->applyFrequencyRange($query, $min, $max);
            return;
        }

        // Если указан пользовательский диапазон
        if ($request->filled('cpu_freq_custom_min') || $request->filled('cpu_freq_custom_max')) {
        ...
        // Если выбран один из диапазонов
        if ($request->has('cpu_freq_range')) {
    ...
    private function applyFrequencyRange($query, $min, $max)
    {
        $query->where(function ($q) use ($min, $max) {
            // Основной запрос для извлечения числового значения из строки
    ...
    private function applyTransformerFilter($query, $request)
    {
        $value = $request->input('transformer');

        if ($value === 'yes') {
            // Фильтр для "Да" - есть трансформатор
            $query->where(function ($q) {
            ...
            // Фильтр для "Нет" - нет трансформатора
            $query->where(function ($q) {
                $q->where('transformer', 'нет')
                    ->orWhere('transformer', 'like', '%нет%')
                    ->orWhere('transformer', 'no')
                    ->orWhere('transformer', '=', 0)
                    ->orWhereNull('transformer'); // если NULL считается как "нет"
            });
        }
        // Если значение пустое - не фильтруем
    }

    private function applyBatteryCapacityFilter($query, $request)
    {
        // Если выбраны конкретные емкости
        if ($request->filled('battery_capacities') && is_array($request->battery_capacities)) {
            ...
            $query->where(function ($q) use ($selectedCapacities) {
                foreach ($selectedCapacities as $capacity) {
                    // Ищем значения, начинающиеся с выбранной емкости
    ...
    private function applyEthernetFilter($query, $request)
    {
        if ($request->filled('ethernet')) {
            $value = $request->input('ethernet');

            if ($value === 'yes') {
                // Ищем все варианты наличия Ethernet
                $query->where(function ($q) {
            ...
            elseif ($value === 'no') {
                // Ищем все варианты отсутствия Ethernet
                $query->where(function ($q) {
                    $q->where('ethernet_lan', 'нет')
                        ->orWhere('ethernet_lan', 'like', 'нет%')
                        ->orWhere('ethernet_lan', 'like', '%нет')
                        ->orWhere('ethernet_lan', 'like', '%нет%')
                        ->orWhereNull('ethernet_lan'); // NULL тоже считается как "нет"
                });
            }
        }
    }

    private function applyUsbPortsFilter($query, $request)
    {
        // Проверяем, есть ли выбранные USB порты
        if ($request->filled('usb_ports') && is_array($request->usb_ports)) {
    ...
    private function applyThunderboltFilter($query, $request)
    {
        if ($request->filled('thunderbolt')) {
            $value = $request->input('thunderbolt');

            if ($value === 'yes') {
                // Ищем все варианты наличия Thunderbolt
                $query->where(function ($q) {
            ...
            elseif ($value === 'no') {
                // Ищем все варианты отсутствия Thunderbolt
                $query->where(function ($q) {
                    $q->where('thunderbolt', 'нет')
                        ->orWhere('thunderbolt', 'like', 'нет%')
                        ->orWhere('thunderbolt', 'like', '%нет')
                        ->orWhere('thunderbolt', 'like', '%нет%')
                        ->orWhereNull('thunderbolt'); // NULL тоже считается как "нет"
                });
            }
            // Если значение пустое - не фильтруем
        }
    ...
    private function applyCyrillicKeyboardFilter($query, $request)
    {
        if ($request->filled('cyrillic_keyboard')) {
            $value = $request->input('cyrillic_keyboard');

            if ($value === 'yes') {
                // Объединяем ВСЕ варианты с "есть"
                $query->where(function ($q) {
