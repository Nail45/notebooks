@php
    function getFirstNotebookImage($slug, $useCache = true) {
        static $cache = [];

//        // Возвращаем из кэша памяти, если есть
//        if ($useCache && isset($cache[$slug])) {
//            return $cache[$slug];
//        }
//
//        $publicFolder = public_path('storage/images/' . $slug);
//
//        // Быстрая проверка без scandir()
//        if (!is_dir($publicFolder)) {
//            $result = asset('storage/images/default.jpg');
//            $cache[$slug] = $result;
//            return $result;
//        }
//
//        // Кэшируем результаты scandir
//        static $dirCache = [];
//        if (!isset($dirCache[$slug])) {
//            $dirCache[$slug] = scandir($publicFolder);
//        }
//
//        $files = $dirCache[$slug];
//
...
//                    $ext === 'gif' || $ext === 'webp') {
//                    $result = asset('storage/images/' . $slug . '/' . $file);
//                    $cache[$slug] = $result;
//                    return $result;
//                }
//            }
            ...
            <div class="product-card bg-white rounded-xl shadow-sm border border-gray-100
                ...
                <a href="/" class="block p-4">
                    ...
                    <div class="aspect-square mb-4 flex items-center justify-center">
                        <img loading="lazy" src="{{ getFirstNotebookImage($notebook->slug) }}"
                             alt="{{ $notebook->title }}"
                    ...
                    <div class="flex items-center justify-between pt-3 border-t border-gray-100">
                        ...
                        <!-- Кнопка в корзину -->
                        <button class="add-to-cart-btn bg-blue-600 text-white px-4 py-2
                                   rounded-lg font-medium hover:bg-blue-700
                                   transition-colors flex items-center space-x-2
                                   transform hover:scale-105 active:scale-95"
                                data-product-id="{{ $notebook->id }}"
                                onclick="addToCart(event, {{ $notebook->id }})">
