<header id="header">
  <div class="relative">
    <!-- Верхняя строка -->
    <div class="bg-gray-800 text-white text-sm">
      <div class="container mx-auto px-4">
        <div class="flex items-center justify-between py-2">
          <!-- Локация -->
          <button class="flex items-center space-x-1 hover:text-gray-300">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16"
                 fill="currentColor">
              <path fill-rule="evenodd" clip-rule="evenodd"
                    d="M17 10L17 10C17 10.2888 16.9755 10.5718 16.9285 10.8472C16.3178 14.5457 12 18 12 18C12 18 7.68203 14.5456 7.07145 10.847C7.02447 10.5717 7 10.2887 7 10V10C7 7.23858 9.23858 5 12 5C14.7614 5 17 7.23858 17 10ZM12 12C13.1046 12 14 11.1046 14 10C14 8.89543 13.1046 8 12 8C10.8954 8 10 8.89543 10 10C10 11.1046 10.8954 12 12 12Z"/>
            </svg>
            <span>г. Минск</span>
          </button>


          <!-- Контакты -->
          <ul class="flex items-center space-x-6">
            <li>
              <a href="https://t.me/online21vekbybot" target="_blank"
                 class="flex items-center space-x-1 hover:text-gray-300">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16"
                     fill="currentColor">
                  <path fill-rule="evenodd" clip-rule="evenodd"
                        d="M21 12C21 16.9706 16.9706 21 12 21C7.02944 21 3 16.9706 3 12C3 7.02944 7.02944 3 12 3C16.9706 3 21 7.02944 21 12ZM12.3229 9.64417C11.4475 10.0083 9.69794 10.7619 7.07426 11.905C6.64821 12.0744 6.42503 12.2401 6.40471 12.4022C6.37038 12.6761 6.71337 12.7839 7.18044 12.9308C7.24397 12.9508 7.3098 12.9715 7.37729 12.9934C7.83681 13.1428 8.45496 13.3176 8.7763 13.3245C9.06779 13.3308 9.39313 13.2106 9.75232 12.964C12.2037 11.3092 13.4691 10.4728 13.5486 10.4548C13.6046 10.4421 13.6823 10.4261 13.7349 10.4729C13.7875 10.5196 13.7824 10.6082 13.7768 10.632C13.7428 10.7768 12.3964 12.0285 11.6997 12.6763C11.4825 12.8782 11.3284 13.0215 11.2969 13.0542C11.2263 13.1275 11.1544 13.1968 11.0853 13.2634C10.6584 13.675 10.3383 13.9836 11.103 14.4876C11.4706 14.7298 11.7646 14.93 12.058 15.1298C12.3785 15.348 12.698 15.5657 13.1115 15.8367C13.2169 15.9058 13.3175 15.9775 13.4155 16.0474C13.7884 16.3132 14.1234 16.552 14.5373 16.514C14.7778 16.4918 15.0262 16.2657 15.1523 15.5912C15.4505 13.9973 16.0366 10.5438 16.172 9.1207C16.1839 8.99602 16.1689 8.83645 16.1569 8.7664C16.145 8.69635 16.1199 8.59655 16.0289 8.52267C15.921 8.43517 15.7546 8.41672 15.6801 8.41803C15.3416 8.424 14.8223 8.60458 12.3229 9.64417Z"/>
                </svg>
                <span>Telegram</span>
              </a>
            </li>
            <li>
              <div class="flex items-center space-x-1 cursor-pointer">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16"
                     fill="currentColor">
                  <g clip-path="url(#clip0_13941_8239)">
                    <mask id="mask0_13941_8239" style="mask-type:luminance"
                          maskUnits="userSpaceOnUse" x="4" y="4" width="16" height="16">
                      <path d="M19.5 4H4V19.853H19.5V4Z" fill="white"/>
                    </mask>
                    <g mask="url(#mask0_13941_8239)">
                      <path
                        d="M14.793 8.82008C14.793 8.82008 15.9204 8.45116 16.8266 7.56567L16.8287 8.30335C16.3408 8.71049 15.6867 8.92778 15.6867 8.92778L14.793 8.82008Z"
                        fill="#0D0D0D"/>
                      <path
                        d="M18.9468 4L18.9448 4.45818C19.0233 7.25522 19.0179 10.0705 18.9265 12.8676C18.9265 12.8475 18.9265 12.8274 18.9265 12.8073L18.9229 14.1219L19.4984 13.7275V4.56955L18.9468 4Z"
                        fill="#0D0D0D"/>
                      <path
                        d="M18.9461 4.45461L18.9462 4L17.15 4.25926C16.991 4.73213 16.7773 5.77098 14.793 6.77332V8.82002C14.793 8.82002 15.9203 8.4512 16.8266 7.56571V13.9595L18.9224 14.122L18.926 12.8074C18.8475 10.0286 18.8548 7.23157 18.9461 4.45461Z"
                        fill="#414141"/>
                      <path
                        d="M18.9467 4.45459C19.027 7.25346 19.0197 10.0706 18.9265 12.8677C18.8461 10.0688 18.8535 7.25163 18.9467 4.45459Z"
                        fill="#ACACAC"/>
                      <path
                        d="M13.8435 18.911L13.1309 16.4316L12.2061 16.473L12.8056 18.5459L13.8435 18.911Z"
                        fill="#820000"/>
                      <path
                        d="M11.6215 14.1312L10.7354 10.9361L11.2397 9.27832L12.6247 14.1312H11.6215Z"
                        fill="#820000"/>
                      <path
                        d="M12.6244 6.30956L9.71918 5.76001L8.87136 8.1992C7.87189 11.272 6.81578 14.3538 5.71582 17.4047C7.72754 16.8003 9.06324 15.557 10.0865 14.1311H9.76488L11.2394 9.27823L11.7072 10.9178C12.6518 8.31059 12.6244 6.30956 12.6244 6.30774V6.30956Z"
                        fill="#FF3F36"/>
                      <path
                        d="M12.67 6.31687L12.6244 6.30957C12.6244 6.30957 12.6518 8.30867 11.7071 10.9177L12.6244 14.1311H10.0864C9.06317 15.557 7.72748 16.8003 5.71576 17.4047C5.41976 18.2226 5.12375 19.0405 4.82227 19.853L8.31401 19.4879L9.13442 16.6104L13.1305 16.4315L13.8431 18.9109L16.3427 18.6389L12.67 6.31687Z"
                        fill="#EB140A"/>
                      <path
                        d="M8.95716 7.93673C8.93887 7.99514 8.91878 8.05175 8.90052 8.11015L9.71904 5.7605L8.47111 6.40126L4 19.1724L4.82247 19.8531C6.11429 15.8675 7.5009 11.8657 8.95533 7.93673H8.95716Z"
                        fill="#B80000"/>
                      <path
                        d="M8.95718 7.93628C7.66535 11.9219 6.2767 15.9239 4.82227 19.853C6.11409 15.8673 7.50274 11.8653 8.95718 7.93628Z"
                        fill="#FDC9C4"/>
                    </g>
                  </g>
                </svg>
                <span>+375 29 302 10 21</span>
              </div>
            </li>
          </ul>

          <!-- Время работы -->
          <div class="flex items-center space-x-2">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16"
                 fill="currentColor">
              <path fill-rule="evenodd" clip-rule="evenodd"
                    d="M5.9 12C5.9 8.63106 8.63106 5.9 12 5.9C15.3689 5.9 18.1 8.63106 18.1 12C18.1 15.3689 15.3689 18.1 12 18.1C8.63106 18.1 5.9 15.3689 5.9 12ZM12 4.5C7.85786 4.5 4.5 7.85786 4.5 12C4.5 16.1421 7.85786 19.5 12 19.5C16.1421 19.5 19.5 16.1421 19.5 12C19.5 7.85786 16.1421 4.5 12 4.5ZM12.6705 9.20439C12.6705 8.81779 12.3571 8.50439 11.9705 8.50439C11.5839 8.50439 11.2705 8.81779 11.2705 9.20439V12.9091C11.2705 13.3509 11.6287 13.7091 12.0705 13.7091H14.2791C14.6657 13.7091 14.9791 13.3957 14.9791 13.0091C14.9791 12.6225 14.6657 12.3091 14.2791 12.3091H12.6705V9.20439Z"/>
            </svg>
            <div class="text-xs">
              <span>контакт-центр</span>
              <div>с 8:00 до 22:00</div>
            </div>
          </div>
        </div>
      </div>
    </div>

    <!-- Основная строка с лого и поиском -->
    <div class="bg-white shadow-md relative">
      <div class="container mx-auto px-4 py-4">
        <div class="flex items-center justify-between">
          <!-- Логотип -->
          <a href="{{route('products.index')}}" class="flex items-center text-gray-800 no-underline">
            <svg width="125" height="42" viewBox="0 0 125 42" fill="none"
                 xmlns="http://www.w3.org/2000/svg">
              <path
                d="M122.036 16.9164H106.625C105.545 16.9164 104.671 16.0422 104.671 14.9618V14.9127C104.671 13.8323 105.545 12.9582 106.625 12.9582H110.446C111.526 12.9582 112.4 12.084 112.4 11.0036V10.9545C112.4 9.87414 111.526 9 110.446 9H4.66535C2.09204 9 0 11.092 0 13.6653V27.9462C0 30.5588 2.1215 32.6803 4.7341 32.6803H114.316C115.396 32.6803 116.27 31.8062 116.27 30.7258V30.6865C116.27 29.6061 115.396 28.732 114.316 28.732H110.505C109.424 28.732 108.55 27.8578 108.55 26.7774C108.55 25.697 107.676 24.8229 106.596 24.8229H98.9152C97.8348 24.8229 96.9606 23.9488 96.9606 22.8684V22.8291C96.9606 21.7487 97.8348 20.8745 98.9152 20.8745H122.045C123.126 20.8745 124 20.0004 124 18.92V18.8807C124 17.7905 123.116 16.9164 122.036 16.9164Z"
                fill="url(#Logo-0.9524345436374753)"></path>
              <path
                d="M18.1515 16.3958H19.7033C19.8408 16.3958 19.9489 16.5038 19.9489 16.6413V28.2605C19.9489 28.4275 20.0864 28.565 20.2533 28.565H23.2883C23.4552 28.565 23.5927 28.4275 23.5927 28.2605V14.6672C23.5927 13.852 22.9249 13.1841 22.1096 13.1841H18.1613C17.9943 13.1841 17.8568 13.3216 17.8568 13.4886V16.1012C17.8568 16.2583 17.9845 16.3958 18.1515 16.3958Z"
                fill="white"></path>
              <path
                d="M33.7472 17.8101L31.2722 24.4006C31.2623 24.43 31.2427 24.4398 31.2132 24.4398C31.1838 24.4398 31.1641 24.4202 31.1543 24.4006L28.6104 17.7905C28.3944 17.2208 27.8444 16.8378 27.2256 16.8378H24.8978C24.6817 16.8378 24.5442 17.0539 24.613 17.2503L28.9444 28.3686C28.9935 28.4864 29.1015 28.5552 29.2292 28.5552H32.8829C33.0008 28.5552 33.1186 28.4766 33.1579 28.3686L37.6956 17.2503C37.7742 17.0539 37.6268 16.8378 37.4206 16.8378H35.1321C34.5232 16.8476 33.9633 17.2307 33.7472 17.8101Z"
                fill="white"></path>
              <path
                d="M57.8291 17.4074L54.4995 21.5817C54.4799 21.6013 54.4504 21.621 54.4209 21.621C54.4013 21.621 54.3817 21.6013 54.3817 21.5817V13.4296C54.3817 13.2921 54.2736 13.1841 54.1361 13.1841H51.1306C50.9931 13.1841 50.8851 13.2921 50.8851 13.4296V28.3194C50.8851 28.457 50.9931 28.565 51.1306 28.565H54.1361C54.2736 28.565 54.3817 28.457 54.3817 28.3194V23.4577C54.3817 23.438 54.4013 23.4184 54.4209 23.4184C54.4406 23.4184 54.4602 23.4282 54.47 23.4478L57.7603 27.9659C58.0354 28.3489 58.4872 28.5748 58.9586 28.5748H62.023C62.19 28.5748 62.2784 28.3882 62.1802 28.2605L58.0452 22.6425C57.9764 22.5541 57.9862 22.4264 58.055 22.338L62.2685 17.1914C62.3766 17.0637 62.2784 16.8673 62.1114 16.8673H58.9782C58.5461 16.8476 58.1139 17.0539 57.8291 17.4074Z"
                fill="white"></path>
              <path
                d="M90.2041 17.8101L87.7781 24.2631C87.7683 24.2925 87.7486 24.3023 87.7192 24.3023C87.6897 24.3023 87.6701 24.2827 87.6602 24.2631L85.0673 17.7807C84.8414 17.2208 84.3012 16.8476 83.6922 16.8476H81.3547C81.1386 16.8476 81.0011 17.0637 81.0796 17.2601L85.7254 28.9579C85.745 29.0168 85.745 29.0856 85.7254 29.1445L84.3994 32.2678C84.311 32.4642 84.4583 32.6901 84.6744 32.6901H87.7781C87.9058 32.6901 88.0138 32.6116 88.0531 32.5035L94.1623 17.2699C94.2408 17.0735 94.0935 16.8574 93.8873 16.8574H91.589C90.9702 16.8476 90.4202 17.2307 90.2041 17.8101Z"
                fill="white"></path>
              <path
                d="M43.4219 16.4547C39.9057 16.4547 37.0574 19.1361 37.0574 22.7014C37.0574 26.2667 39.9057 28.948 43.4219 28.948C45.9854 28.948 48.1855 27.5239 49.1971 25.3631C49.2953 25.1667 49.148 24.9309 48.9221 24.9309H45.73C45.6416 24.9309 45.5532 24.9702 45.4943 25.039C45.0425 25.5988 44.2077 25.8738 43.4121 25.8738C42.037 25.8738 41.0156 25.0586 40.5834 23.9095H46.2113H48.5391H48.8239C49.3248 23.9095 49.7373 23.5166 49.7668 23.0157C49.7766 22.9077 49.7766 22.7996 49.7766 22.6818C49.7963 19.1557 46.9381 16.4547 43.4219 16.4547ZM40.7406 21.4737C40.6718 21.4737 40.6227 21.4049 40.6424 21.3362C41.0942 20.2558 42.0862 19.5093 43.4121 19.5093C44.797 19.5093 45.897 20.2165 46.2211 21.346C46.2408 21.4049 46.1917 21.4737 46.1229 21.4737H40.7406Z"
                fill="white"></path>
              <path
                d="M65.0017 28.565H65.0508C66.0428 28.565 66.8482 27.7596 66.8482 26.7676C66.8482 25.7756 66.0428 24.9702 65.0508 24.9702H65.0017C64.0097 24.9702 63.2043 25.7756 63.2043 26.7676C63.2142 27.7596 64.0097 28.565 65.0017 28.565Z"
                fill="white"></path>
              <path
                d="M75.333 16.386C74.0561 16.4646 72.8874 16.9556 72.0623 17.7021C72.023 17.7316 71.9739 17.7512 71.9248 17.7512V13.4886C71.9248 13.3216 71.7873 13.1841 71.6203 13.1841H68.7033C68.5363 13.1841 68.3988 13.3216 68.3988 13.4886V28.2605C68.3988 28.4275 68.5363 28.565 68.7033 28.565H71.5418C71.7087 28.565 71.8462 28.4275 71.8462 28.2605V27.6221C71.9052 27.6221 71.9739 27.6516 72.0132 27.6909C72.8088 28.4275 74.1151 28.8891 75.333 28.9382C78.7804 29.0757 81.3537 26.2372 81.3537 22.6621C81.3537 19.1066 78.8197 16.1601 75.333 16.386ZM74.8419 25.864C73.182 25.864 71.8266 24.43 71.8266 22.6621C71.8266 20.8942 73.1722 19.4602 74.8419 19.4602C76.5116 19.4602 77.8572 20.8942 77.8572 22.6621C77.8474 24.43 76.5018 25.864 74.8419 25.864Z"
                fill="white"></path>
              <path
                d="M17.0213 17.4467C17.0213 15.0208 15.3025 12.693 11.472 12.7028C6.31557 12.7028 5.90305 16.769 5.89323 17.928C5.89323 18.095 6.03073 18.2325 6.19771 18.2325H9.24246C9.38978 18.2325 9.51747 18.1244 9.53711 17.9771C9.62551 17.3682 9.98891 15.9047 11.4425 15.9047C12.8765 15.9047 13.348 16.8673 13.348 17.5941C13.348 18.6057 12.9649 19.1361 10.7943 21.179L7.22899 24.3416C6.35485 25.1176 5.85394 26.2176 5.85394 27.3864V28.2212C5.85394 28.3882 5.99145 28.5257 6.15842 28.5257H15.607C16.3731 28.5257 17.0017 27.9069 17.0017 27.131V25.3533C17.0017 25.2158 16.8936 25.1077 16.7561 25.1077H11.5702C11.4818 25.1077 11.4327 24.9899 11.5113 24.9309L13.5444 23.222C15.4596 21.4737 17.0213 19.8334 17.0213 17.4467Z"
                fill="white"></path>
              <defs>
                <linearGradient id="Logo-0.9524345436374753" x1="-5.70822" y1="23.2924" x2="127.227"
                                y2="18.2967"
                                gradientUnits="userSpaceOnUse">
                  <stop offset="0.2873" stop-color="#BB0088"></stop>
                  <stop offset="0.5525" stop-color="#D9006D"></stop>
                  <stop offset="1" stop-color="#F20061"></stop>
                </linearGradient>
              </defs>
            </svg>
          </a>

          {{-- resources/views/partials/search.blade.php --}}

          <!-- Поиск -->
          <div class="flex-1 max-w-2xl mx-4 relative search-container">
            <div class="relative">
              {{-- Поле ввода --}}
              <input type="text"
                     id="search-input"
                     placeholder="Поиск по ноутбукам"
                     autocomplete="off"
                     class="w-full px-4 py-2 pl-10 pr-20 border border-gray-300 rounded-lg
                      focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent
                      transition-all duration-200">

              {{-- Иконка поиска --}}
              <button type="button"
                      id="search-button"
                      class="absolute left-3 top-1/2 transform -translate-y-1/2 hover:text-pink-500 transition">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20"
                     fill="currentColor" class="text-gray-400 hover:text-pink-500">
                  <path
                    d="M16.4406 15.3108C18.392 12.8035 18.2153 9.17663 15.9107 6.87197C13.4147 4.37601 9.36794 4.37601 6.87197 6.87197C4.37601 9.36794 4.37601 13.4147 6.87197 15.9107C9.17662 18.2153 12.8034 18.392 15.3108 16.4406L17.3058 18.5368C17.6152 18.8619 18.1317 18.8683 18.449 18.551C18.7555 18.2445 18.7616 17.7495 18.4628 17.4356L16.4406 15.3108Z"/>
                </svg>
              </button>

              {{-- Индикатор загрузки --}}
              <div id="search-loader"
                   class="absolute right-3 top-1/2 transform -translate-y-1/2 hidden">
                <svg class="animate-spin h-5 w-5 text-pink-500"
                     xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                  <path class="opacity-75" fill="currentColor"
                        d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z">
                  </path>
                </svg>
              </div>

              {{-- Кнопка очистки --}}
              <button id="clear-search"
                      class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400
                       hover:text-gray-600 transition hidden">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                     stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                </svg>
              </button>
            </div>

            <div id="search-results"
                 class="absolute z-50 w-full mt-2 bg-white border border-gray-200 rounded-lg shadow-xl
                max-h-[32rem] overflow-y-auto hidden">
            </div>
          </div>

          <!-- Иконки действий -->
          <div class="flex items-center space-x-6">

            <!-- Аккаунт -->
            <a href="{{route('profile.edit')}}"
               class="flex flex-col items-center text-gray-700 cursor-pointer hover:text-pink-500">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"
                   fill="currentColor">
                <path fill-rule="evenodd" clip-rule="evenodd"
                      d="M14.9 7.25C14.9 8.71355 13.7136 9.9 12.25 9.9C10.7865 9.9 9.60001 8.71355 9.60001 7.25C9.60001 5.78645 10.7865 4.6 12.25 4.6C13.7136 4.6 14.9 5.78645 14.9 7.25ZM14.888 10.5824C15.8701 9.80391 16.5 8.60052 16.5 7.25C16.5 4.90279 14.5972 3 12.25 3C9.9028 3 8.00001 4.90279 8.00001 7.25C8.00001 8.60051 8.62993 9.8039 9.61198 10.5824C7.47858 11.5771 6 13.7409 6 16.25V17C6 17.8284 6.67157 18.5 7.5 18.5H17C17.8284 18.5 18.5 17.8284 18.5 17V16.25C18.5 13.7409 17.0214 11.5771 14.888 10.5824ZM16.9 16.25V16.9H7.6V16.25C7.6 13.6819 9.68188 11.6 12.25 11.6C14.8181 11.6 16.9 13.6819 16.9 16.25Z"/>
              </svg>
              <span class="text-xs mt-1">
                            @auth
                  {{Auth::user()->name}}
                @else
                  Войти
                @endauth
                                   </span>
            </a>

            <a href="{{ route('basket.index') }}"
               class="relative inline-flex items-center justify-center px-4 py-2 font-medium text-white bg-brand-pink rounded-lg hover:bg-brand-pink-hover transition-colors">
              <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"/>
              </svg>

              <!-- Счетчик товаров -->
              @php
                use App\Models\Basket;

                $cartCount = 0;

                try {
                    if (auth()->check()) {
                        // Для авторизованных пользователей
                        $cartCount = Basket::where('user_id', auth()->id())->sum('count');
                    } else {
                        // Для гостей
                        $cart = session('cart', []);
                        // Проверяем, что это массив
                        if (is_array($cart)) {
                            $cartCount = array_sum($cart);
                        } else {
                            // Если по какой-то причине не массив, приводим к массиву
                            $cartCount = array_sum((array) $cart);
                        }
                    }
                } catch (Exception $e) {
                    // В случае ошибки показываем 0
                    $cartCount = 0;
                }
              @endphp

              <span id="cart-badge"
                    class="cart-count absolute -top-2 -right-2 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center {{ $cartCount > 0 ? '' : 'hidden' }}">
        {{ $cartCount > 0 ? $cartCount : '' }}
    </span>

              <!-- Для мобильной версии -->
              <span class="ml-2 hidden md:inline">Корзина</span>
            </a>

          </div>
        </div>
      </div>
    </div>
  </div>
</header>
